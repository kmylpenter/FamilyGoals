...
# Termux Android - Dodatkowe instrukcje

## Proste komendy (powiedz słowo, ja robię resztę):

- "stan"       → Na starcie sesji (czytam STATE.md)
- "changelog"  → Po zadaniu (piszę co zrobiliśmy)
- "devlog"     → Na koniec sesji (piszę podsumowanie)
- "pomoc"      → Pokazuję welcome screen

## Co robię automatycznie:

- Aktualizuję STATE.md co 30-60 min podczas pracy
- Dodaję blokery do STATE.md gdy się pojawią
- Git hook aktualizuje logi przy COMMIT
- Gdy widzę ważną decyzję → PYTAM czy utworzyć ADR

## Komendy bash (w terminalu):

- p=wybierz projekt  np=nowy  cc=claude+wakelock
- md=czytaj  mds=szukaj  mde=edytuj  mdl=lista
- gs=commit  gp=push  gl=pull  gst=status
- wl=lock  wu=unlock  komendy=pomoc
