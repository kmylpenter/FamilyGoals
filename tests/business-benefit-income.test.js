...
test('cykliczna korzyść wchodzi w przychody Męża na wykresie (w zakresie)', () => {
  ...
  run(`dataManager.addBusinessCost({ name: 'Leasing', amount: 600, isRecurring: true, recurringMonths: 1, activeFrom: '2026-05' })`);
  const trend = run('dataManager.getTrendByOwner(3)'); // okno kwi–cze (bez trwającego lipca)
  assert.equal(trend[0].husbandIncome, 0, 'kwiecień: przed zakresem');
  assert.equal(trend[1].husbandIncome, 600, 'maj: start zakresu');
  assert.equal(trend[2].husbandIncome, 600, 'czerwiec: w zakresie');
...
test('jednorazowa korzyść z datą trafia w miesiąc realizacji (Mąż)', () => {
  const { run } = dm('2026-07-15T12:00:00');
  run(`dataManager.addBusinessCost({ name: 'Laptop', amount: 4000, lastPurchaseDate: '2026-05-10T10:00:00.000Z' })`);
  const trend = run('dataManager.getTrendByOwner(2)'); // okno maj–cze
  assert.equal(trend[0].husbandIncome, 4000, 'maj: miesiąc zakupu');
  assert.equal(trend[1].husbandIncome, 0, 'czerwiec: już nie');
});
...
test("okno 'auto' sięga najwcześniejszej korzyści (zakup z 2023)", () => {
  ...
  assert.equal(trend.length, 40, `marzec 2023 → czerwiec 2026 (ostatni zamknięty) = 40 mies., jest ${trend.length}`);
